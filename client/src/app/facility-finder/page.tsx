...
// Mock types and functions for now
interface Location {
  lat: number
  lng: number
}
...
const searchNearbyFacilities = async (location: Location, radius: number, type?: string) => {
  ...
  return {
    status: "OK",
    facilities: type && type !== "all" ? mockFacilities.filter(f => f.type === type) : mockFacilities
  }
}

const getDirectionsUrl = (location: Location, name: string) => {
...
export default function FacilityFinderPage() {
  ...
  const fetchNearbyFacilities = async (location: Location, type?: 'hospital' | 'clinic' | 'pharmacy') => {
    setLoading(true)
    try {
      const response = await searchNearbyFacilities(location, 10000, type)
